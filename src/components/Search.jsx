...
const Search = () => {
  ...
  const searchproductByCategory = async (categoryid= "" , brandid="")  => {
    ...
    const newcat = { categoryid: categoryid, brandid: brandid  }; // pass productid
    const postData = {
      headers: { "content-type": "application/json" },
      method: "post",
        ...
        <div className='container-fluid mt-3'>
         <div className='row '>
            <div className="col-lg-2 col-12 mt-3">
                 ...
                 <div className='overflow-y-scroll   shop' style={{height:"250px"}}>
                  {
                     brand.map((item,index) =>{
                       return(
                         <div key={index}>
                          <p className='d-flex align-items-center gap-2 m-2' onClick={()=>{searchproductByCategory("",item.brandid)}}> {item.brandname}</p>
                         </div>
                       )
                     })
                  }
                ...
                <div className='overflow-y-scroll  shop' style={{height:"250px"}}>
                  {
                     category.map((item,index) =>{
                       return(
                         <div key={index} >
                          <p className='d-flex align-items-center gap-2 m-2'  onClick={()=>{searchproductByCategory(item.catid,"")}}> {item.categoryname}</p>
                         </div>
                       )
                     })
                  }
              ...
              <input
            type="text"
            className="form-control w-25 border-2 border-dark mb-lg-2 w-100"
            placeholder="Search...."
                  ...
                  suggestedProduct.map((result,index)=>{
                     return(
                       <div key={index} onClick={suggestionValue.bind(this,result)}> 
                         <PiClockClockwise className='mx-2' />{result.name}
                       </div>
                     )
                  })
                }
               ...
               <div className='d-flex flex-wrap gap-3 mt-4'>
                  {
                    productList
                      ...
                      .map((item, index) => {
                        ...
                        return (
                          <ProductCard key={index} productname={item.productname} details={item.details} price={item.price} productid={item.productid} url={item.url} photo={item.photo} categoryid = {item.categoryid}/>
                        )
                      })
                  }
               </div>
